...
async function forceEmptyWorkflows() {
    console.log('🗑️  Vidage forcé de tous les fichiers non-vides optimisés...\n');
    ...
    try {
        ...
        for (const filename of files) {
            ...
            try {
                ...
                // Vérifier si le fichier contient la structure "emptied-for-rag" ou juste "{}"
                if (content.includes('"emptied-for-rag"') || content.includes('"Large workflow emptied for RAG optimization"') || content.trim() === '{}') {
                    console.log(`🗑️  Vidage forcé de ${filename} (${originalSize.toLocaleString()} chars)`);
                    
                    // Fichier complètement vide (rien du tout)
                    const emptyContent = '';
        ...
        console.log('\n✅ Vidage forcé terminé avec succès !');
        
    } catch (error) {
        console.error('❌ Erreur lors du vidage forcé:', error);
    }
}
